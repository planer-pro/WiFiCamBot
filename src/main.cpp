...
// DNS и гасит точку доступа. Сама библиотека после успеха портала этого НЕ
// делает (известный косяк — порт 80 остаётся занят, наши серверы тогда не
// поднялись бы). В блокирующем режиме stopConfigPortal лишь ставит флаг —
// потому сначала переключаем режим.
static void wmClosePortal(WiFiManager &wm)
{
...
static void connectWiFi()
{
  ...
  // autoConnect с выключенным порталом — ТОЛЬКО попытка сохранённой сети;
  // сам портал поднимаем явно: после успешного выбора сети его нужно
  // свернуть вручную (wmClosePortal выше).
  WiFiManager wm;
  wmConfigure(wm);
  wm.setEnableConfigPortal(false);
  if (!wm.autoConnect(WIFI_AP_NAME))
  {
    // Не получилось за WIFI_CONNECT_TIMEOUT_MS — настроечная точка доступа
    // WIFI_AP_NAME со страницей выбора сети; выбранное сохранится в NVS
    // и плата подключится уже к нему.
...
// 0 = отключено): рост скважности до установленной мощности растянут на N мс,
// а стоп и ЛЮБОЕ снижение мощности всегда мгновенные — торможение не должно
...
// Варианты разгона, мс набора мощности до установленного значения (0 —
// отключено, старт сразу на полную). Порядок совпадает с <option> селектов
// в INDEX_HTML.
...
// Шаг пропорционален «живому» диапазону [точка страгивания ... цель]: N мс —
// время разгона до УСТАНОВЛЕННОЙ мощности, а не до 100 % (при мощности 50 %
// и разгоне 500 мс танк выходит на неё за полные полсекунды, а не за 250 мс).
static void motorsTick(uint32_t dtMs)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    {
      // цель всегда выше точки страгивания; если вдруг нет (точку подняли
      // одновременно с командой) — выходим на цель за один шаг
      const uint32_t span = (g_chTarget[i] > lo) ? (g_chTarget[i] - lo) : 1;
      uint32_t step = span * dtMs / g_chAccelMs;
      if (step == 0)
      {
...
// Качество, свет (вкл/выкл + цвет), мощности (свои у каждого вида
// управления), разгоны, точка страгивания и вид управления
...
<script>
  ...
  // Разгон: время плавного набора мощности до установленного значения при
  // старте (0 — отключено, сразу на полную). Стоп всегда мгновенный. Список
  // значений обязан совпадать с таблицей MOTOR_ACCEL_STEPS в прошивке.
  ...
  // команды motor=; мощность у клавиш — пара активного вида управления,
  // в трекпаде разгон отключён (как показывают поля).
...
static esp_err_t set_handler(httpd_req_t *req)
{
  ...
  if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK)
  {
    ...
    else if (httpd_query_key_value(query, "start", val, sizeof(val)) == ESP_OK)
    {
      ...
      // диапазона хода); 0 — как раньше, шкала с нуля. val обязан начинаться
      // с цифры — иначе atoi дал бы 0 и мусор прошёл бы как «валидный ноль»
      int pct = atoi(val);
      if (val[0] >= '0' && val[0] <= '9' && pct >= 0 && pct <= 90)
      {
        g_motorStartPct = pct;
        applyMotors();
